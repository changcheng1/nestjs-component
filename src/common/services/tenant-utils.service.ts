 ...
 * @LastEditTime: 2025-08-21 11:45:00
...
export class TenantUtilsService {
   ...
   * @returns 包含租户ID的数据对象
   */
  static ensureTenantId<T extends Record<string, any>>(
    data: T,
    fallbackTenantId: string = '1',
  ): T & { tenantId: string } {
    const tenantId = data.tenantId || fallbackTenantId;
   ...
   * @returns 租户ID
   */
  static extractTenantId(body?: any, query?: any, params?: any): string {
    return body?.tenantId || query?.tenantId || params?.tenantId || '1';
  }
}
